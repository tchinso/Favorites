(() => {
  ...
  function bindCanvas() {
    cv.addEventListener("pointerdown", (event) => { cv.setPointerCapture(event.pointerId); const point = toCard(event); pointers.set(event.pointerId, point); if (pointers.size === 2 && state.tool === "photo" && state.photo && !(drag && drag.mode !== "photo")) { const [a, b] = [...pointers.values()]; pinch = { d: Math.hypot(a.x - b.x, a.y - b.y), ang: Math.atan2(b.y - a.y, b.x - a.x), scale: state.photo.scale, rot: state.photo.rot, cx: (a.x + b.x) / 2, cy: (a.y + b.y) / 2, px: state.photo.x, py: state.photo.y }; drag = null; curStroke = null; return; } if (pointers.size > 1) return; if (state.tool === "draw") { pushHistory(); curStroke = { type: state.brush.type, color: state.brush.color, size: state.brush.size, points: [point] }; state.strokes.push(curStroke); inkDirty = true; render(); return; } if (state.tool === "text" || state.tool === "sticker") { const handle = handleHit(point); if (handle === "delete") { removeItem(state.selected); return; } if (handle === "resize") { const item = state.selected; drag = { mode: "resize", item, d0: Math.hypot(point.x - item.x, point.y - item.y), s0: item.size, a0: Math.atan2(point.y - item.y, point.x - item.x), r0: item.rot }; return; } const item = hitItem(point); if (item) { selectItem(item); drag = { mode: "move", item, dx: point.x - item.x, dy: point.y - item.y }; return; } selectItem(null); return; } if (state.tool === "photo" && state.photo) drag = { mode: "photo", dx: point.x - state.photo.x, dy: point.y - state.photo.y }; });
    ...
    cv.addEventListener("wheel", (event) => { if (!state.photo || state.tool !== "photo") return; event.preventDefault(); const point = toCard(event); const photo = state.photo; const nextScale = Math.max(.02, Math.min(12, photo.scale * (event.deltaY < 0 ? 1.08 : 1 / 1.08))); const factor = nextScale / photo.scale; photo.x = point.x - (point.x - photo.x) * factor; photo.y = point.y - (point.y - photo.y) * factor; photo.scale = nextScale; clampPhoto(); syncPhotoUI(); render(); }, { passive: false });
  }
  ...
  function bindControls() {
    ...
    document.querySelectorAll("[data-cheki-mode]").forEach((element) => {
      const activateMode = () => { if (state.tool !== element.dataset.chekiMode) setTool(element.dataset.chekiMode); };
      element.addEventListener("pointerdown", activateMode);
      element.addEventListener("focusin", activateMode);
    });
