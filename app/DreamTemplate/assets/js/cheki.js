(() => {
  ...
  function makeChips(id, list, current, onPick) { const box = ui(id); if (!box) return; box.innerHTML = ""; list.forEach(([value, label]) => { const button = document.createElement("button"); button.className = "cheki-chip"; button.type = "button"; button.textContent = label; button.dataset.val = value; button.setAttribute("aria-pressed", String(value === current)); button.onclick = () => { markPressed(id, value); onPick(value); }; box.appendChild(button); }); }
  function makeSwatches(id, colors, current, onPick) { const box = ui(id); if (!box) return; box.innerHTML = ""; colors.forEach((color) => { const button = document.createElement("button"); button.className = "cheki-sw"; button.type = "button"; button.style.background = color; button.dataset.val = color; button.title = color; button.setAttribute("aria-pressed", String(color === current)); button.onclick = () => { markPressed(id, color); onPick(color); }; box.appendChild(button); }); const input = document.createElement("input"); input.type = "color"; input.value = current; input.title = "직접 고르기"; input.oninput = (event) => { markPressed(id, "__custom__"); onPick(event.target.value); }; box.appendChild(input); }
  ...
  function setTool(tool, clickedTab) {
    document.querySelectorAll(".cheki-tab").forEach((tab) => tab.setAttribute("aria-selected", "false")); clickedTab.setAttribute("aria-selected", "true"); document.querySelectorAll(".cheki-pane").forEach((pane) => pane.classList.remove("active")); ui(`pane-${tool}`).classList.add("active"); state.tool = tool; cv.classList.toggle("drawing", tool === "draw");
  ...
  function bindControls() {
    ...
    ui("drop").onclick = () => ui("file").click(); ui("file").onchange = (event) => { readFile(event.target.files[0]); event.target.value = ""; };
    ["dragenter", "dragover"].forEach((name) => ui("drop").addEventListener(name, (event) => { event.preventDefault(); ui("drop").classList.add("over"); })); ["dragleave", "drop"].forEach((name) => ui("drop").addEventListener(name, (event) => { event.preventDefault(); ui("drop").classList.remove("over"); })); ui("drop").addEventListener("drop", (event) => { event.stopPropagation(); readFile(event.dataTransfer.files[0]); });
    ...
    ui("stDrop").onclick = () => ui("stFile").click(); ui("stFile").onchange = (event) => { [...event.target.files].forEach(readSticker); event.target.value = ""; }; ["dragenter", "dragover"].forEach((name) => ui("stDrop").addEventListener(name, (event) => { event.preventDefault(); ui("stDrop").classList.add("over"); })); ["dragleave", "drop"].forEach((name) => ui("stDrop").addEventListener(name, (event) => { event.preventDefault(); ui("stDrop").classList.remove("over"); })); ui("stDrop").addEventListener("drop", (event) => { event.stopPropagation(); [...event.dataTransfer.files].forEach(readSticker); }); ui("stSize").oninput = (event) => { state.sticker.size = +event.target.value; ui("stSizeVal").textContent = event.target.value; if (state.selected?.type === "image") { state.selected.size = +event.target.value; render(); } }; ui("stOpacity").oninput = (event) => { state.sticker.opacity = +event.target.value / 100; ui("stOpacityVal").textContent = `${event.target.value}%`; if (state.selected?.type === "image") { state.selected.opacity = state.sticker.opacity; render(); } }; ui("stFlip").onclick = () => { if (!state.selected || state.selected.type !== "image") { toast("먼저 스티커를 눌러주세요"); return; } state.selected.flip = !state.selected.flip; render(); }; ui("stDel").onclick = () => { if (!state.selected || state.selected.type !== "image") { toast("먼저 스티커를 눌러주세요"); return; } removeItem(state.selected); };
    ...
    document.querySelectorAll(".cheki-tab").forEach((tab) => { tab.onclick = () => setTool(tab.dataset.tab, tab); }); ui("undoBtn").onclick = undo; ui("resetBtn").onclick = () => { if (confirm("지금 만든 체키를 모두 지우고 처음부터 시작할까요?")) reset(); }; ui("saveBtn").onclick = savePng; ui("gifBtn").onclick = saveGif;
  }

  function savePng() { const quality = state.quality; const output = document.createElement("canvas"); output.width = CARD_W * quality; output.height = CARD_H * quality; renderTo(output.getContext("2d"), quality, false); const date = new Date(); const pad = (value) => String(value).padStart(2, "0"); const link = document.createElement("a"); link.download = `cheki_${date.getFullYear()}${pad(date.getMonth() + 1)}${pad(date.getDate())}_${pad(date.getHours())}${pad(date.getMinutes())}.png`; link.href = output.toDataURL("image/png"); link.click(); toast("체키를 저장했어요"); }
  ...
  window.CardStudioCheki = { mount, reset };
})();
