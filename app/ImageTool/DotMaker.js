...
const ALPHA_CAPABLE_INPUT_TYPES = new Set(['image/png', 'image/webp', 'image/avif']);

const elements = {
  form: document.getElementById('dm_converter-form'),
  fileInput: document.getElementById('dm_image'),
  convertButton: document.getElementById('dm_convert'),
  downloadButton: document.getElementById('dm_download'),
  status: document.getElementById('dm_status'),
  originalPreview: document.getElementById('dm_original-preview'),
  resultCanvas: document.getElementById('dm_result-canvas'),
  palette: document.getElementById('dm_palette')
};
...
function hasTransparencyInCanvas(canvas) {
...
function shouldUseAlphaMode(file, pngColorType, alphaEnabled, sourceCanvas) {
  if (!alphaEnabled) {
    return false;
  }
  if (!ALPHA_CAPABLE_INPUT_TYPES.has(file.type)) {
    return false;
  }
  if (file.type === 'image/png' && (pngColorType === 3 || pngColorType === 4 || pngColorType === 6)) {
    return true;
  }
  return hasTransparencyInCanvas(sourceCanvas);
}
...
async function runConversion() {
  ...
  if (!SUPPORTED_INPUT_TYPES.has(file.type)) {
    setStatus('PNG/JPEG/WEBP/AVIF 파일만 지원합니다.', 'error');
    return;
  }
  ...
  try {
    ...
    const alphaMode = shouldUseAlphaMode(file, pngColorType, options.alpha, sourceCanvas);
...
window.addEventListener('opencv-ready-dotmaker', markOpenCvReady);
